...
if (isset($_FILES['music_file'])) {
    ...
    if ($file_type == 'audio/mpeg' || $file_type == 'audio/mp3') {
        ...
        $sql = "INSERT INTO music (user_id, username, music_name, music_path) VALUES ('$user_id', '$username', '$file_name_without_extension', '$music_path')";
        // Выполнение запроса
        if ($conn->query($sql) === TRUE) {
            // Перенаправление пользователя на страницу music_list.php после успешной загрузки
            header("Location: music_list.php");
            exit(); // Убедитесь, что прекращаете выполнение скрипта после перенаправления
        } else {
