...
$video_description = $_POST['video_description']; // Получаем описание видео изформы
$video_source = $_POST['video_source']; // Получаем источник видео из формы
$video_link = ($_POST['video_link']) ? $_POST['video_link'] : null; // Получаем ссылку на видео из формы, если она была указана
$video_path = null; // Переменная для хранения пути к видео
...
if ($video_source == 'file') {
    ...
    $target_file = $target_dir . basename($_FILES["video_file"]["name"]); // Coздаем полный путь к загруженному файлу
    ...
    if (!empty($video_link)) {
        ...
        //strpod- позиция первого вхождения построки.  если построка не найдена, то вернет false.
        if (strpos($video_link, 'youtube.com') !== false || strpos(
            $video_link,
            'youtu.be'
        ) !== false) {
            // Разбиваем URL-адрес, чтобы получить идентификатор видео
            //explode(разделитель, входная_строка)-разбивает строку разделителя.Возвращает массив строк.
            $video_id = '';
        ...
        elseif (strpos($video_link, 'rutube.ru') !== false) {
            ...
            $video_path = "https://rutube.ru/play/embed/" .
                basename(parse_url($video_link, PHP_URL_PATH));
        }
    } else {
...
if ($video_path !== null) {
    ...
    $sql = "INSERT INTO videos (user_id, username, video_name, video_description, 
video_source, video_path) VALUES ('$user_id', '$username', '$video_name', 
