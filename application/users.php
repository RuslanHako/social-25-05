...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['button-upd'])) {
    ...
    if ($pass_first !== $pass_second) {
        ...
        // Проверяем, существует ли пользователь с таким паролем, исключая текущего пользователя
        $check_password_query = "SELECT * FROM users WHERE password='$password' AND id != $id";
